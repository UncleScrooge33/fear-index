...
exports.handler = async (event) => {
  try {
    ...
    // Buckets fixes & formats :
    // 1H : 1 min ; 1D : 30 min ; 7D : 8 h ; 90D : 3 j ; 1Y : 2 sem
    const cfg = {
      ...
      '1Y':  { spanMs: 365*24*60*1000*24, stepMs: 14*24*60*60*1000,fmt: d=>d.toISOString().slice(0,10) },
    ...
    // 3) Filtre fenêtre [start, endClosed] et normalisation
    const rows = (Array.isArray(subs) ? subs : [])
      .map(s => {
        const t = Date.parse(s.created_at);
        const raw = s.data?.value ?? s.data?.choice; // support des deux clés
        const v = Number(raw);
        return { t, v };
      })
    ...
    // 4) Buckets fixes + moyenne + carry-forward
    const buckets = [];
    ...
    for (const b of buckets) {
      const avg = b.n ? (b.sum / b.n) : prev; // carry-forward quand pas de votes
      prev = avg;
